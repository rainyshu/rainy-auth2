...
public class GlobalTransactionManager {
    ...
    @Value("${transaction.pointcut.path:}")
    ...
    public Advisor txAdviceAdvisor() {
        ...
        StringBuilder builder = getPathBuilder(paths);
        pointcut.setExpression(builder.toString());
        //返回aop配置：整合切面（切入点集合） 和  配置好的事务通知（也就是事务拦截操作）
        Advisor advisor = new DefaultPointcutAdvisor(pointcut, getTxAdvice());
        return advisor;
    }

    private StringBuilder getPathBuilder(List<String> paths) {
        //设置切入点的路径
        StringBuilder builder = new StringBuilder();
        if (paths.size() == 0) {
            builder.append("execution(* com.rainy.core..service..*.*(..))");
            return builder;
        }
